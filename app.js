...
const compare = (human) => {
  ...
  document.getElementById('grid-dinos').style.display = 'block'
}

const obtenerDatos = () => {
  let height, weight;
  if (document.getElementById('metric').checked) {
    height = Math.round(parseFloat(document.getElementById('height-metric').value) / 2.21)
    weight = Math.round(parseFloat(document.getElementById('weight-metric').value) * 2.54)
  } else {
...
compareButton.addEventListener('click', () => {
  if (document.getElementById('name').value == '') return alert('required name')
  if (document.getElementById('diet').value == '') return alert('required diet')
  if (document.getElementById('metric').checked) {
    if (!(
      document.getElementById('height-metric').value && document.getElementById('weight-metric').value 
    )) {
      return alert('required height, weight')
    }
  } else {
    if (!(
      document.getElementById('inches').value &&
      ...
      return alert('required inches, feet, weight')
    }
  }
  const human = obtenerDatos()
  compare(human)
})

repeatButton.addEventListener('click', () => {
  document.getElementById('grid-dinos').style.display = 'none'
